...
class CalculateViewController: UIViewController {
    ...
    @IBAction private func tappedCalculateButton(_ sender: Any) {
        guard let number1: Float = Float(number1TextField.text ?? "") else {
            alert(message: CalculateViewController.number1IsNil)
            return
        }
        guard let number2: Float = Float(number2TextField.text ?? "") else {
            alert(message: CalculateViewController.number2IsNil)
            return
        }
        guard number2 != 0 else {
            alert(message: CalculateViewController.number2IsZero)
            return
        }
